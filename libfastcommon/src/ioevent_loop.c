...
static void deal_ioevents(IOEventPoller *ioevent)
{
	...
	IOEventEntry *pEntry;
	//循环处理每一个事件(在call epoll_wait时,已经将epoll_event放进了IOEventPoller中的events了)
	for (ioevent->iterator.index=0; ioevent->iterator.index < ioevent->iterator.
            count; ioevent->iterator.index++)
	{
		//获取具体的事件类型(read,write)
		event = IOEVENT_GET_EVENTS(ioevent, ioevent->iterator.index);
		//此处的Entry就是在注册事件时，放入的fd和callback方法
		pEntry = (IOEventEntry *)IOEVENT_GET_DATA(ioevent,ioevent->iterator.index);
        if (pEntry != NULL) {
        	//调用事件回调方法(首次调用事件:storage_recv_notify_read)
