...
struct nio_thread_data
{
	...
	int pipe_fds[2];//什么鬼?读写通道.pipe_fds[0]-read,pepe_fds[1]-write
	struct fast_task_info *deleted_list;//删除(已完成)的task列表?
...
struct fast_task_info
{
	...
	int size;   //alloc size		任务数据的(最大值)
	int length; //data length		本次任务要处理的长度
	int offset; //current offset	本次任务当前已处理的数据的偏移量
